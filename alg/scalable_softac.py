...
class rl_alg_scalable_softac:
    ...
    def __init__(self, env_obj, cfg, surface=None):
        ...
        self.start_set = cfg['start_set']
        self.goal_set = cfg['goal_set']
        ...
        self.target_entropy = -np.log(1.0 / 4) * 0.1
    ...
    def rl_start(self, mode="Train"):
        ...
        for ii in range(self.num_UAV):
            action = self._agent[ii].agent_start(state[ii], self.state_input[ii],
                                                 actor_obs_list[ii], 0, self.goal[ii])
    ...
    def rl_episode(self, max_steps_this_episode=100, idx=0):
        ...
        if self._num_episodes % 1000 == 0 and self.mode == "Train":
            ...
            with open('Data/eps_records_v1_ssac' + self.label, 'wb') as f:
    ...
    def policy_update(self, policy_update=False):
        ...
        if policy_update:
            ...
            expected_values = (reward_batch - self.alpha * log_probs.detach() + self.gamma * next_values).detach()
            ...
            # Alpha Loss
            entropy_loss = -torch.mean(self.log_alpha * (self.target_entropy + log_probs.detach()))
    ...
    def save_parameters(self, idx):
        # torch.save(self.actor.state_dict(), 'Data/MAAC_v8_' + self.label + '_trainedActor_ssac_' + str(idx))
        # torch.save(self.critic.state_dict(), 'Data/MAAC_v8_' + self.label + '_trainedCritic_ssac_' + str(idx))
        torch.save(self.actor.state_dict(), 'Data/MAAC_v8_trainedActor_ssac' + self.label)
        torch.save(self.critic.state_dict(), 'Data/MAAC_v8_trainedCritic_ssac' + self.label)
